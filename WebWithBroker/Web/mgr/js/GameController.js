  ...
  return 0;
	...
	var gameSuccessFunction = function(data) {
		...
		if (data['data']['status'] == 'success') {
            ...
            for (i = 0; i < data['data']['gamestat'].length; i++) {
            	...
            	if(data['data']['gamestat'][i]['userType'] == 'pbr'){
            		if(!$scope.pbrs.includes(data['data']['gamestat'][i]['userId'])){
            			pbr={};
            			pbr['name'] = data['data']['gamestat'][i]['userId'];
            			pbr['times'] = 0;
            			$scope.pbrs.push(pbr);
            		}
            ...
            for (i = 0; i < $scope.batmsg.length; i++){
            	...
            	for (j = 0; j < $scope.pbrs.length; j++){
            		if($scope.batmsg[i].includes($scope.pbrs[j]['name'])){
            			$scope.pbrs[j]['times'] = $scope.pbrs[j]['times'] + 1
            		}
            	}
            }
	...
	$scope.switchbtn = function(){
		if($scope.switch == true){
			...
			$scope.subwinners = $scope.sbrs;
			$scope.subwinners.sort(compare);
			
			$scope.pubwinners = $scope.pbrs;
			$scope.pubwinners.sort(compare);
			//console.log($scope.sbrs);
			//console.log($scope.subwinners)
			$("#honor").modal('show');
